...
def main():
    ...
    parser.add_argument('--sam_ckpt', type=str, default='/data/gjx/project/SAMUS/SAMUS/checkpoints/sam_vit_b_01ec64.pth', help='Pretrained checkpoint of SAM')
    ...
    for epoch in range(opt.epochs):
        ...
        model.train()
        train_losses = 0
        for batch_idx, (datapack) in tqdm(enumerate(trainloader), desc = 'Training Batches', unit = 'batch', leave = False):
        ...
        if epoch % opt.eval_freq == 0:
            ...
            print('epoch [{}/{}], val loss:{:.4f}'.format(epoch + 1, opt.epochs, val_losses))
            print('epoch [{}/{}], val dice:{:.4f}'.format(epoch + 1, opt.epochs, mean_dice))
            if args.keep_log:
            ...
            save_path = opt.save_path + args.modelname + opt.save_path_code + '%s' % timestr + '_' + str(epoch + 1) + '_' + f'{mean_dice:.4f}'
