...
def visual_segmentation(seg, image_filename, j, opt):
    # print("data_path",opt.data_path)
    # print("image_filename",image_filename)
    # tt = os.path.join(opt.data_path + '/test' + '/img', "patient"+image_filename)
    # print("tt",tt)
    # print("mode:",opt.mode)     #train
    #===========================val / test===========================================
    img_ori = cv2.imread(os.path.join(opt.data_path + '/test' + '/img', image_filename + '/', str(j).zfill(4) + '.png'))
    # print(os.path.join(opt.data_path + '/test' + '/img', image_filename + '/', str(j).zfill(4) + '.png'))
    img_ori0 = cv2.imread(os.path.join(opt.data_path + '/test' + '/img', image_filename + '/', str(j).zfill(4)+ '.png'))
    # print(img_ori0)
    overlay = img_ori * 0
    img_r = img_ori[:, :, 0]
    ...
    seg0 = seg[0, :, :]
    # print("seg0.shape",seg0.shape)  # 256 256
    ...
    fulldir = opt.visual_result_path + "/" + opt.modelname + "-video" + "/" + image_filename + '/'
    if not os.path.isdir(fulldir):
        os.makedirs(fulldir)
    #============查看分割结果===============
    # seg_dir = fulldir + "segmentation/"
    # if not os.path.isdir(seg_dir):
    #     os.makedirs(seg_dir)
    # seg_filename = image_filename.replace(".png", "_seg.png")
    # cv2.imwrite(os.path.join(seg_dir, seg_filename), seg0 * 255)
    #================================
    cv2.imwrite(fulldir + str(j).zfill(4) + '.png', img)
