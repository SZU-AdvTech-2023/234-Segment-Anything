...
def get_click_prompt(datapack, opt):
    if 'pt' not in datapack:
        ...
        imgs, pt, masks = generate_click_prompt(datapack['image'], datapack['low_mask'])
    else:
        pt = datapack['pt']
    ...
    # print("pt1",coords_torch.shape)
    labels_torch = torch.as_tensor(point_labels, dtype=torch.int, device=opt.device)
    # print("pt2",labels_torch.shape)
    if len(pt.shape) == 2:
        coords_torch, labels_torch = coords_torch[None, :, :], labels_torch[None, :]
    # print("pt1", coords_torch.shape)
    # print("pt2", labels_torch.shape)
    coords_torch = coords_torch.squeeze(0)
    labels_torch = labels_torch.squeeze(0)
