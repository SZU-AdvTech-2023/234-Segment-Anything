...
from .common import LayerNorm2d, Adapter, MLPBlock, AugAdapter
import math
import numpy as np
import warnings
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            ...
            # print(f"pos_embed{pos_embed.shape}\nx{x.shape}")  #pos_embed(1,32,32,768)  x(4,3,256,256)
            x = x + pos_embed
...
class PatchEmbed2(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)  # b c h w
        # print("x.shape:",x.shape)  #(4,3,256,256)
        #======================================================
        B, C, H, W = x.shape
...
class PatchEmbed0(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("Ax.shape:",x.shape)  #(4,3,264,264)
        x = self.proj(x)
        # B C H W -> B H W C
        x = x.permute(0, 2, 3, 1)
        # print("AAx.shape:",x.shape) #(4,32,32,768)
        return x
