...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        # 在第29行，检查x的形状，如果维度为3，则添加一个新的维度
        if x.dim() == 3:
            x = x.unsqueeze(0)
        # print("x.size", x.size())  # 应该为（1，256，32，32）
...
class UnetResSEBlock(nn.Module):
    ...
    def forward(self, inp):
        # print("inp1",inp.shape) #256 32 32
        residual = inp
        out = self.conv1(inp)
        out = self.norm1(out)
        # print("inp2",inp.shape) #256 32 32
        out = self.lrelu(out)
        out = self.conv2(out)
        out = self.norm2(out)
        # print("inp3",inp.shape) # 256 32 32
        out = self.se(out)
        if self.downsample:
            residual = self.conv3(residual)
            residual = self.norm3(residual)
        # print("residual",residual.shape)  # 256 32 32
        out += residual
        out = self.lrelu(out)
        return out
...
class UpBlock(nn.Module):
    ...
    def forward(self, inp, skip):
        ...
        # skip = skip.repeat(4,1,1,1)
        # print(f"inp{inp.shape}\nskip{skip.shape}") # inp 4 64 128 128   skip 4 128 64 64
...
class PIMMDecoder(nn.Module):
    def __init__(
            self,
            endoder_transformer_dim: int = 256,
    ...
    def forward(
        ...
        for d in range(len(self.conv_blocks_context)):
            # print("image_embedding",image_embedding.shape)  # 3 256 32 32
            # print("self.conv_blocks_context",type(self.conv_blocks_context))
            # print("d",d)
            # print("len_image",len(image_embeddings)) # 3
            # print("len_conv",len(self.conv_blocks_context))  #  3
            # print("image_embeddings size:", image_embeddings.size())
            embed = self.conv_blocks_context[d](image_embeddings[0])
            if d == 0:
                embed = embed.expand(mask_embeddings.size(0), -1, -1, -1)
                # print(f"embed{embed.shape}\nmask_embeddings{mask_embeddings.shape}")  # 4 256 32 32  # 4 256 32 32
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        # mask_embedding = mask_embedding.repeat(4, 1, 1, 1)
        # image_embeddings = image_embeddings.repeat(4, 1, 1, 1)
        # print("mask_embedding", mask_embedding.shape)  # 1 256 32 32(test)  4 256 32 32（train)
        # print("image_embedding", image_embeddings.shape)  # 1 256 32 32(test)  4 256 32 32 (train)
        masks = self.pimm(mask_embedding, image_embeddings)
